...
def extract_features(df, encoder, device, num_frames, crop_size):
    features = []
    labels = []
    
    with torch.no_grad():
        for i, row in tqdm(df.iterrows(), total=len(df)):
            v = load_video(row['path'], num_frames, crop_size)
            if v is None:
                continue
            ...
            features.append(z.squeeze(0).cpu().half())
...
def main():
    ...
    parser.add_argument('--lrs', type=str, default="1e-3,3e-4,1e-4", help="Comma-separated learning rates")
    parser.add_argument('--wds', type=str, default="0.05,0.01,0.001", help="Comma-separated weight decays")
    parser.add_argument('--zip_params', action='store_true', help="If set, zips lrs and wds instead of full grid search")
    parser.add_argument('--epochs', type=int, default=30, help="Number of epochs to train the probe")
    ...
    print("Loading full UCF-101 dataframes...")
    train_df = pd.read_csv('/a/mm/VJEPA2/data/ucf101/ucf101_train.csv', sep=' ', header=None, names=['path', 'label'])
    val_df = pd.read_csv('/a/mm/VJEPA2/data/ucf101/ucf101_val.csv', sep=' ', header=None, names=['path', 'label'])
    
    print(f"Train samples: {len(train_df)}, Val samples: {len(val_df)}")
    
    print("Extracting FULL training features...")
    X_train, y_train = extract_features(train_df, encoder, device, num_frames, crop_size)
    
    print("Extracting FULL validation features...")
    X_test, y_test = extract_features(val_df, encoder, device, num_frames, crop_size)
    ...
    learning_rates = [float(x) for x in args.lrs.split(',')]
    weight_decays = [float(x) for x in args.wds.split(',')]
    
    if args.zip_params:
        assert len(learning_rates) == len(weight_decays), "LRs and WDs must be same length for zip_params"
        param_grid = list(zip(learning_rates, weight_decays))
    else:
    ...
    print(f"Starting Hyperparameter Sweep for Attentive Probe ({len(param_grid)} combinations)...")
    
    for lr, wd in param_grid:
        print(f"\n--- Testing LR: {lr}, WD: {wd}, Epochs: {epochs} ---")
        
        probe = AttentiveProbe(embed_dim=X_train.shape[-1], num_classes=101).to(device)
        ...
        acc = correct / total
        print(f"Accuracy for LR={lr}, WD={wd}: {acc*100:.2f}%")
        ...
        all_results.append(sweep_res)
        
        if acc > best_acc:
            best_acc = acc
            best_params = sweep_res
            print(f"*** New Best Accuracy: {best_acc*100:.2f}% ***")
    
    print(f"\n========================================")
    print(f"[{args.name}] Best Swept UCF-101 Accuracy: {best_acc*100:.2f}%")
    print(f"Best Params: {best_params}")
    print(f"========================================")

    results = {
        "name": args.name,
        "dataset": "UCF-101 (Full)",
        "task": "frozen-encoder + attentive probe sweep (101-class)",
        "train_probe_samples": len(X_train),
        "val_samples": len(X_test),
        "top1_accuracy_percent": round(best_acc * 100, 2),
        "best_params": best_params,
        "all_sweep_results": all_results,
