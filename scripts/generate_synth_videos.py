"""
Generate a tiny synthetic video dataset for V-JEPA2 training dry-runs.

Creates N short mp4 clips of moving colored blobs, plus a CSV
(<abs_path> <label>) matching the format that `VideoDataset` expects.

This is ONLY for pipeline debug. Real experiments use SSv2.
"""
...
def _write_video(path: str, frames_thwc_uint8: torch.Tensor, fps: int) -> None:
    """Write a TxHxWx3 uint8 tensor as mp4. Try torchvision first, then imageio."""
    try:
        ...
        print(f"  torchvision.io write failed ({e}); falling back to imageio.")
...
def make_clip(num_frames: int, h: int, w: int, seed: int) -> torch.Tensor:
    """Render a TxHxWx3 uint8 tensor with a moving colored blob on noise."""
...
def main():
    ...
    parser.add_argument("--csv", type=str, required=True,
                        help="CSV path to write <abs_path> <label>.")
    parser.add_argument("--num-clips", type=int, default=24)
    parser.add_argument("--num-frames", type=int, default=32,
                        help="Frames per clip (must be >= fpc*fps buffer).")
    ...
    for i in range(args.num_clips):
        clip = make_clip(args.num_frames, args.height, args.width, seed=args.seed + i)
        clip_path = out_dir / f"synth_{i:04d}.mp4"
