...
def extract_features(df, encoder, device, num_frames, crop_size):
    ...
    with torch.no_grad():
        for i, row in tqdm(df.iterrows(), total=len(df)):
            ...
            if features is None:
                features = torch.zeros((len(df), *z.shape), dtype=torch.float16)
                
            features[valid_idx] = z
            labels[valid_idx] = row['label']
            valid_idx += 1
...
def main():
    ...
    base_dir = '/a/mm/VJEPA2/data/KathaBench-V1'
    print(f"Loading KathaBench-V1 data from {base_dir}...")
    
    df = pd.read_csv(os.path.join(base_dir, 'labels.csv'))
    train_df = df[df['split'] == 'train']
    test_df = df[df['split'] == 'test']
        
    print(f"Train samples: {len(train_df)}, Val samples: {len(test_df)}")
    ...
    X_test, y_test = extract_features(test_df, encoder, device, num_frames, crop_size)
    ...
    print(f"Training Attentive Probe (8 classes) for {args.epochs} epochs...")
    probe = AttentiveProbe(embed_dim=X_train.shape[-1], num_classes=8).to(device)
    ...
    print(f"\n========================================")
    print(f"[{args.name}] KathaBench-V1 Accuracy: {acc*100:.2f}%")
    print(f"========================================")

    results = {
        "name": args.name,
        "dataset": "KathaBench-V1 (8-class)",
        "task": "frozen-encoder + attentive probe (8-class)",
