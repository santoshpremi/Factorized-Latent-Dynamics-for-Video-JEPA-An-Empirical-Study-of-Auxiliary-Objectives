...
def extract_features(df, encoder, device, num_frames, crop_size, max_samples=None):
    features = []
    labels = []
    ...
    with torch.no_grad():
        for i, row in tqdm(df.iterrows(), total=len(df)):
            ...
            features.append(z.squeeze(0).cpu())
...
def main():
    ...
    config_path = '/a/mm/VJEPA2/configs/train_ucf101_4gpu_baseline.yaml'
    checkpoint_path = '/a/mm/VJEPA2/runs/train_ucf101_4gpu_baseline/latest.pth.tar'
    
    encoder, cfg, num_frames = load_model(config_path, checkpoint_path, device)
    crop_size = cfg['data']['crop_size']
    
    print("Loading UCF-101 dataframes...")
    ...
    X_train, y_train = extract_features(train_df, encoder, device, num_frames, crop_size, max_samples=2000)
    
    print("Extracting validation features...")
    X_test, y_test = extract_features(val_df, encoder, device, num_frames, crop_size, max_samples=500)
    ...
    print("Training Attentive Probe (101 classes)...")
    ...
    # Keep X_train and X_test on CPU, and only move batches to GPU
    batch_size = 16
    epochs = 100
    ...
    print(f"\n========================================")
    print(f"Baseline V-JEPA UCF-101 Action Recognition Accuracy: {acc*100:.2f}%")
    print(f"========================================")
    print("Random chance would be ~1% (101 classes)")
