...
def copy_code_folder(code_folder, ignore_patterns, ignore_paths):
    ...
    # torchrun launches one process per GPU; all run this function. Only rank 0
    # must copy or we race and get FileExistsError on .../runs/.../code.
    local_rank = int(os.environ.get("LOCAL_RANK", "0"))
    ready_sentinel = os.path.join(code_folder, "vjepa2", "app", "main_distributed.py")

    if local_rank == 0:
        if not os.path.exists(code_folder):
            shutil.copytree(".", code_folder, ignore=ignore_func)
        elif not os.path.exists(ready_sentinel):
        ...
        while time.time() < deadline:
            ...
            raise RuntimeError(
                f"Timeout waiting for rank-0 code copy at {code_folder}"
            )
